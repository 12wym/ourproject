...
nn_error_e RKEngine::LoadModelFile(const char *model_file)
{
    ...
    /**将yolov8需要的模型信息拷贝到model_output_shapes中来方便后处理*/
    model_output_shapes = (rknn_tensor_attr *)malloc(io_num.n_output * sizeof(rknn_tensor_attr));
    memcpy(model_output_shapes, output_attrs, io_num.n_output * sizeof(rknn_tensor_attr));
...
nn_error_e RKEngine::Run(std::vector<tensor_data_s> &inputs, std::vector<tensor_data_s> &outputs, bool want_float)
{
    ...
    //copy rknn outputs to yolov8

    // for (int i = 0; i < output_num_; ++i)
    // {
    //     // 将rknn_output转换为自定义的tensor_data_s
    //     rknn_output_copy_to_B(rknn_outputs[i], model_output_data[i]);
