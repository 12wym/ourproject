...
#define LABEL_NALE_TXT_PATH "./model/coco_80_labels_list.txt"
    ...
    static int process(int8_t* input, const int* anchor, int grid_h, int grid_w, int height, int width, int stride,
                   std::vector<float>& boxes, std::vector<float>& objProbs, std::vector<int>& classId, float threshold,
                   int32_t zp, float scale, int PROP_BOX_SIZE)
{
  // printf("%d %d %d %d %d %d\n", grid_h, grid_w, height, width, stride, OBJ_CLASS_NUM);
  int    validCount = 0;
  int    grid_len   = grid_h * grid_w;
  float  thres      = unsigmoid(threshold);
  int8_t thres_i8   = qnt_f32_to_affine(thres, zp, scale);
  for (int a = 0; a < 3; a++) {
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        // int8_t box_confidence = input[(PROP_BOX_SIZE * a + 4) * grid_len + i * grid_w + j];
        int8_t box_confidence = input[(PROP_BOX_SIZE * a ) * grid_len + (i * grid_w + j)*PROP_BOX_SIZE +4];
        if (box_confidence >= thres_i8) {
          
          // int     offset = (PROP_BOX_SIZE * a) * grid_len + i * grid_w + j;
          int     offset = (PROP_BOX_SIZE * a ) * grid_len + (i * grid_w + j)*PROP_BOX_SIZE ;
          int8_t* in_ptr = input + offset;
          float   box_x  = sigmoid(deqnt_affine_to_f32(*in_ptr, zp, scale)) * 2.0 - 0.5;
          ...
          for (int k = 0; k < PROP_BOX_SIZE - 5; ++k) {
            // printf("%d %d\n", k, OBJ_CLASS_NUM);
  ...
  return validCount;
                 ...
                 std::vector<float> &qnt_scales, detect_result_group_t *group)
    {
        ...
        validCount0 = process(input0, (int *)anchor0, grid_h0, grid_w0, model_in_h, model_in_w, stride0, filterBoxes,
                              objProbs,
                              classId, conf_threshold, qnt_zps[0], qnt_scales[0] , 10);
        ...
        validCount1 = process(input1, (int *)anchor1, grid_h1, grid_w1, model_in_h, model_in_w, stride1, filterBoxes,
                              objProbs,
                              classId, conf_threshold, qnt_zps[1], qnt_scales[1] , 10);
        ...
        validCount2 = process(input2, (int *)anchor2, grid_h2, grid_w2, model_in_h, model_in_w, stride2, filterBoxes,
                              objProbs,
                              classId, conf_threshold, qnt_zps[2], qnt_scales[2] , 10);
