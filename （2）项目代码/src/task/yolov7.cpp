...
nn_error_e YOLOV7::LoadModel(const char* model_path){
    ...
    //获取模型的输出存入,在yolov7中输出3个分支，每个分支是一个4维度的向量
...
// 图像预处理
nn_error_e YOLOV7::Preprocess(const cv::Mat &img, const std::string process_type, cv::Mat &image_letterbox)
{
...
// 后处理
nn_error_e YOLOV7::Postprocess(const cv::Mat &img, std::vector<Detection> &objects)
{
